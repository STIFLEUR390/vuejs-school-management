    ...
    <div>
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">{{ $t('update_class') }}</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">{{ $t('update_class') }}</li>
                        </ol>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            ...
                            <form @submit.prevent="updatedClass">
                                <div class="card-body">
                                    <div class="form-group">
                                        <label for="name">{{ $t('class_name') }}</label>
                                        <input type="text" class="form-control" id="name" name="name" v-model="form.name">
                                        <small class="form-text text-muted">{{ $t('provide_class_name') }}</small>
                                    </div>
                                </div>
                                <!-- /.card-body -->

                                <div class="card-footer">
                                    <button type="submit" class="btn btn-primary">{{ $t('update_class') }}</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            async getClass(){
                this.axios.get('/superadmin/crud/'+this.$route.params.id+'?getData=class')
                    .then((response) => {
